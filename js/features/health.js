...
function plantDiversity(pid){
 let set=new Set();
 db.entries.filter(x=>x.pid===pid&&days(7).includes(x.date)).forEach(x=>{if(x.plantGroupId)set.add(x.plantGroupId);else if(Array.isArray(x.plantKeys))x.plantKeys.forEach(k=>set.add(k));else if(x.plantKey)set.add(x.plantKey);});
 return set.size;
}
function plantDiversityToday(pid,date=localDate()){let set=new Set();db.entries.filter(x=>x.pid===pid&&x.date===date).forEach(x=>{if(x.plantGroupId)set.add(x.plantGroupId);else if(Array.isArray(x.plantKeys))x.plantKeys.forEach(k=>set.add(k));else if(x.plantKey)set.add(x.plantKey);else if(typeof allFoods==='function'){const f=allFoods()[x.food];if(f&&(Number(f.veg)>0||Number(f.fruit)>0))set.add(plantGroupId(x.food));}});return set.size}
