...
function runDeepAudit(){
  ...
  try{
    ok('Versión única',VERSION==='7.8.1'&&NF_RUNTIME.version==='7.8.1'&&window.NF_APP_VERSION==='7.8.1',VERSION);
    ok('Namespace nuevo',KEY==='nutrifamilia_v7_8_1',KEY);
    ok('Cache actual',window.NF_CACHE_NAME==='nutrifamilia-v7.8.1','nutrifamilia-v7.8.1');
    ok('Catálogo base',typeof allFoods==='function'&&Object.keys(allFoods()).length>=212,Object.keys(allFoods()).length);
    ok('Catálogo visible',Array.isArray(NF_FOOD_VISIBLE_SET)&&NF_FOOD_VISIBLE_SET.length===198,'198 visibles');
...
function NutriFamiliaDeepAudit(){
  ...
  try{
    ...
    add('Identidad de release',VERSION==='7.8.1'&&NF_RUNTIME.version==='7.8.1'&&window.NF_APP_VERSION==='7.8.1',VERSION);
    add('Namespace local nuevo',KEY==='nutrifamilia_v7_8_1',KEY);
    add('Cache PWA nueva',window.NF_CACHE_NAME==='nutrifamilia-v7.8.1',window.NF_CACHE_NAME);
    add('Base nutricional',Object.keys(fs).length===253,Object.keys(fs).length+' alimentos');
    add('Catálogo visible',Array.isArray(NF_FOOD_VISIBLE_SET)&&NF_FOOD_VISIBLE_SET.length===198,'198 visibles');
