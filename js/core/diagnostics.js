function runSelfTest(){const R=[],ok=(name,pass,detail)=>R.push({name,pass:!!pass,detail:String(detail||'')});try{
 ok('Versión',VERSION==='7.7.0'&&NF_RUNTIME.version==='7.7.0',VERSION);ok('renderHome presente',typeof window.renderHome==='function','función de dashboard');ok('Namespace independiente',KEY==='nutrifamilia_v7_7_0'&&!Object.prototype.hasOwnProperty.call(window,'LEGACY_KEYS'),'namespace sin migración implícita');ok('Favoritos',Array.isArray(db.favoriteFoods),'colección');ok('Voz progresiva',typeof startVoiceMeal==='function','Web Speech');ok('Barcode lookup',typeof lookupBarcode==='function','Open Food Facts opcional');ok('Colecciones',Array.isArray(db.profiles)&&Array.isArray(db.entries)&&Array.isArray(db.weights)&&Array.isArray(db.recipes),'arrays');ok('Comida múltiple',typeof createMealDraft==='function'&&typeof addMealDraftItem==='function'&&typeof mealDraftTotals==='function'&&typeof groupMealEntries==='function','varios alimentos por comida');ok('Catálogo visual',typeof foodIcon==='function'&&foodCategories().length===12&&['carnes-proteinas','panificados','cereales-legumbres','infusiones','bebidas','comidas-preparadas'].every(id=>foodCategories().some(c=>c.id===id)),'12 categorías nuevas e iconos');const taxonomy=window.NF_FOOD_CATEGORY_MAP||{},visible=window.NF_FOOD_VISIBLE_CATEGORY_MAP||{};ok('Taxonomía V7.7',Object.keys(taxonomy).length>=30&&Object.keys(visible).length===196,'taxonomía explícita + catálogo visible');ok('Bebidas ampliadas',['Agua','Gaseosa común','Gaseosa sin azúcar','Vino tinto','Cerveza','Mate dulce con azúcar','Mate con edulcorante'].every(n=>!!allFoods()[n]),'bebidas e infusiones');ok('Funciones UI',typeof setup==='function'&&typeof showTab==='function'&&typeof render==='function'&&typeof addWeight==='function'&&typeof deleteWeight==='function'&&typeof runSelfTest==='function','presentes');
...
function runDeepAudit(){
  ...
  try{
    ok('Versión única',VERSION==='7.7.0'&&NF_RUNTIME.version==='7.7.0'&&window.NF_APP_VERSION==='7.7.0',VERSION);
    ok('Namespace nuevo',KEY==='nutrifamilia_v7_7_0',KEY);
    ok('Cache actual',window.NF_CACHE_NAME==='nutrifamilia-v7.7.0','nutrifamilia-v7.7.0');
    ok('Catálogo base',typeof allFoods==='function'&&Object.keys(allFoods()).length>=212,Object.keys(allFoods()).length);
    ok('Catálogo visible',Array.isArray(NF_FOOD_VISIBLE_SET)&&NF_FOOD_VISIBLE_SET.length===196,'196 visibles');
    ok('Categorías',Array.isArray(NF_FOOD_CATEGORIES)&&NF_FOOD_CATEGORIES.length===12,'12 categorías');
    ...
    ok('Comidas por roles',typeof createMealDraft==='function'&&typeof mealDraftRoleSections==='function','Principal/Guarnición/Extra');
...
function NutriFamiliaDeepAudit(){
  ...
  try{
    ...
    add('Identidad de release',VERSION==='7.7.0'&&NF_RUNTIME.version==='7.7.0'&&window.NF_APP_VERSION==='7.7.0',VERSION);
    add('Namespace local nuevo',KEY==='nutrifamilia_v7_7_0',KEY);
    add('Cache PWA nueva',window.NF_CACHE_NAME==='nutrifamilia-v7.7.0',window.NF_CACHE_NAME);
    add('Base nutricional',Object.keys(fs).length===251,Object.keys(fs).length+' alimentos');
    add('Catálogo visible',Array.isArray(NF_FOOD_VISIBLE_SET)&&NF_FOOD_VISIBLE_SET.length===196,'196 visibles');
    add('Categorías',Array.isArray(NF_FOOD_CATEGORIES)&&NF_FOOD_CATEGORIES.length===12,'12 categorías');
    ...
    add('Comidas por roles',typeof createMealDraft==='function'&&typeof mealDraftRoleSections==='function','Principal/Guarnición/Extra');
